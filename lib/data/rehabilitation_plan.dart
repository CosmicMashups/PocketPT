...
Future<RehabilitationPlan?> generateRehabilitationPlanFromCSV() async {
  ...
  final filteredExercises = data.where((row) {
    final csvMuscle = row[col('Muscle_Involved')].toString().toLowerCase();
    final inputMuscle = UserAssess.specificMuscle.toLowerCase().trim();
    
    final csvPainLevel = row[col('Pain_Level')].toString().toLowerCase();
    final inputPainLevel = UserAssess.painLevel.toLowerCase().trim();
    
    final csvGoal = row[col('Functional_Goal')].toString().toLowerCase();
    final inputGoal = UserAssess.rehabGoal.toLowerCase().trim();

    // Check if CSV muscle contains input muscle (partial match)
    bool muscleMatch = csvMuscle.contains(inputMuscle);
    bool painLevelMatch = csvPainLevel == inputPainLevel;
    bool goalMatch = csvGoal == inputGoal;
